...
public class WechatUtil {
    ...
    private static Map<Object, Object> buildRequsetMapParam(String out_trade_no, BigDecimal bdOrderMoney, BigDecimal bdRefundMoney) {
        ...
        params.put("appid", PropertiesUtils.getInstance().getProperty("appid"));
        //微信支付分配的商户号
        params.put("mch_id", PropertiesUtils.getInstance().getProperty("wx.mchId"));
        ...
        //操作员帐号, 默认为商户号
        params.put("op_user_id", "1521795051");
        //签名前必须要参数全部写在前面
        params.put("sign", arraySign(params, PropertiesUtils.getInstance().getProperty("wx.paySignKey")));
        return params;
    }
    ...
    public static String sendSSLPostToWx(String mapToXml, SSLConnectionSocketFactory sslcsf) {
        ...
        HttpPost httPost = new HttpPost(PropertiesUtils.getInstance().getProperty("wx.refundUrl"));
    ...
    public Map<String, Object> wxRefundquery(String out_trade_no, String out_refund_no) {
        Map<Object, Object> params = new HashMap<Object, Object>();
        //微信分配的公众账号ID（企业号corpid即为此appId）
        params.put("appid", PropertiesUtils.getInstance().getProperty("wx.appid"));
        //微信支付分配的商户号
        params.put("mch_id", PropertiesUtils.getInstance().getProperty("mchId"));
        //随机字符串，不长于32位。推荐随机数生成算法
        params.put("nonce_str", CharUtil.getRandomString(16));
        //商户侧传给微信的订单号
        params.put("out_trade_no", out_trade_no);
        ...
        params.put("sign", arraySign(params, PropertiesUtils.getInstance().getProperty("wx.paySignKey")));
        String mapToXml = MapUtils.convertMap2Xml(params);
        HttpPost httPost = new HttpPost(PropertiesUtils.getInstance().getProperty("wx.refundqueryUrl"));
        httPost.addHeader("Connection", "keep-alive");
        httPost.addHeader("Accept", "*/*");
        httPost.addHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
        httPost.addHeader("Host", "api.mch.weixin.qq.com");
    ...
    public static String requestOnce(final String url, String data) throws Exception {
        ...
        httpPost.addHeader("User-Agent", "wxpay sdk java v1.0 " + "1521795051");
